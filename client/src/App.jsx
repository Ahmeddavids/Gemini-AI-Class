...
function App() {
  ...
  const uploadImage = async () => {
    try {
      ...
      // Trigger the analyzeImage function with the prompt after successful upload
      if (response.data.message === "Uploaded successfully") {
        analyzeImage('gemini', image);
      }
    } catch (err) {
      console.error(err);
      setError('Something went wrong');
  ...
  const analyzeImage = async (endpoint, filePath) => {
    try {
      setLoading(true);
      setError('');
      setResponse('');
